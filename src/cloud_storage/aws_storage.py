...
class SimpleStorageService:
    """this class is used for interacting with AWS S3 storage it provides functionalties like file management,data uploades and data retrevels in S3 buckets"""

    def __init__(self):
        """intialize this class with the resource and client from S3Client class (the 4th code line file) """
        s3_clinet=S3Client()
    ...
    def read_object(object_name:str,decode:bool=True,make_readable:bool=False)->Union[StringIO,str]:
        ...
        try:
            #read and decode the content of object if decode is true
            func=(
                lambda:object_name.get()["Body"].read().decode()
                if decode else object_name.get()["Body"].read()
            )
            #convert to StringIO if make_reaable is true
    ...
    def get_bucket(self,bucket_name:str)->Bucket:
        ...
        """
        logging.info("entered the get_bucket function from SimpleStorageService class")
        try:
            bucket=self.s3_resource.Bucket(bucket_name)
            logging.info("exited the get_bucket method feom simplestorageservice")
            return bucket
        except Exception as e:
            raise MyException(e,sys) from e
    ...
    def load_model(self,model_name:str,bucket_name:str,model_dir:str)->object:
        """
