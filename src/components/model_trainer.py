...
class ModelTrainer:
    def __init__(self,data_transformation_artifact:DataTransformationArrifact,model_trainer_config:ModelTrainerConfig):
        """
    ...
    def get_model_object_and_report(self,train:np.array,test:np.array)->Tuple[object,object]:
        ...
        """
        try:
            logging.info("training random forest classfier with given parameters")
    ...
    def initiate_model_trainer(self)->ModelTrainerArtifact:
        ...
        try:
            ...
            #loading the model and metrics report
            trained_model,metric_artifact=self.get_model_object_and_report(train=train_arr,test=test_arr)
            logging.info("model and metrics loaded succesfully")
            ...
            logging.info("pre processing object loaded succesfully")
            if metric_artifact.accuracy_score<self.model_trainer_config.expected_accuracy:
                logging.info("accuracy is less than expected")
                raise Exception("No model found with desired accuracy")
            logging.info("saving the model")
            my_model=MyModel(preprocessing_object=preprocessing_obj,trained_model_object=trained_model)
            save_object(self.model_trainer_config.trained_model_file_path,my_model)
            logging.info("Saved final model object that includes both preprocessing and the trained model")

            #model trainer artifact that has to be returned
